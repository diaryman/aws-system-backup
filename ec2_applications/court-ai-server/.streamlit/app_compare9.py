...
# 🛠️ FIXED: Added Private EC2 Model
MODELS = {
    "Claude 3.5 Sonnet": {"type": "bedrock", "id": "anthropic.claude-3-5-sonnet-20240620-v1:0", "icon": "🔶", "color": "#d97757"},
    
    # ✅ NEW: โมเดลบนเครื่อง EC2 ส่วนตัว
    "DeepSeek R1 (Private EC2)": {
        "type": "deepseek_self_hosted", 
        "id": "deepseek-r1:8b", 
        "icon": "🏰", 
        "color": "#2ecc71"
    },
...
MODEL_PRICING = {
    ...
    "deepseek-r1:8b": [0.0, 0.0], # EC2 จ่ายรายชั่วโมง ไม่คิดค่า token (ใส่ 0 ไว้ก่อน)
...
def save_to_sheet(username, q, r_left, r_right):
    print("🔄 Attempting to save to Google Sheet...")
    try:
        sheet = get_sheet_client()
        if sheet is None:
            print("❌ Failed to get sheet client.")
            return
...
def load_history_from_sheet(target_username):
    try:
        ...
        # Filter by username if column exists
...
def call_single_model(model_name, prompt, context, citations_dict, temperature=0.5):
    ...
    try:
        ...
        # ✅ NEW: Logic สำหรับเชื่อมต่อ Private EC2
        elif cfg["type"] == "deepseek_self_hosted":
            # เชื่อมต่อตรงไปที่ IP ของ EC2
            custom_client = OpenAI(
                base_url="http://98.84.25.78:11434/v1",  # 👈 ใช้ IP ล่าสุด
                api_key="ollama",
                timeout=120.0
            )
        ...
        if "402" in error_msg or "Insufficient Balance" in error_msg:
            answer = "⚠️ ยอดเงินเครดิต API หมด (Insufficient Balance) กรุณาเติมเงิน"
             ...
             answer = f"⚠️ เชื่อมต่อ Server EC2 ไม่ได้ (IP: 98.84.25.78) ตรวจสอบการเปิดเครื่องและ Security Group"
        else:
            answer = f"⚠️ เกิดข้อผิดพลาด: {error_msg}"
    
    elapsed = time.time() - start_time
    # EC2 Cost is fixed per hour, approximated per query as 0 here for calculation simplicity
...
with st.sidebar:
    ...
    # ปรับ Default index ให้ชี้ไปที่ DeepSeek Private (ตัวที่ 3 ในลิสต์)
    m_right = st.selectbox("🦁 ฝั่งขวา", model_keys, index=2) 
...
# ==============================================================================
# ✅ ส่วนที่แก้ไข: ใช้ st.expander เพื่อให้คลิกหัวข้อแล้วโชว์เนื้อหา
# ==============================================================================
def display_result_box(res_data):
    ...
    # ⚠️ ปิด div ของ response-card ก่อนเริ่มส่วน Citations
    st.markdown("</div></div>", unsafe_allow_html=True)

    if res_data.get("citations"):
        # แสดงหัวข้อรวม
        st.markdown(f"<div style='margin-top: 10px; margin-bottom: 5px; font-size: 0.85rem; opacity: 0.8;'>📚 <b>เอกสารอ้างอิง ({len(res_data['citations'])}):</b></div>", unsafe_allow_html=True)
        
        # วนลูปสร้าง Expander แยกรายไฟล์ (คลิกหัวข้อ -> โชว์เนื้อหา)
    ...
    # Footer Stats
    st.markdown(f"""
        <div style="margin-top:5px; font-size:0.85rem; color:#2ecc71 !important; text-align:right;">

...
def render_result_card(res_data, kb_name):
    ...
    # Inject Copy Button (using separate component to run JS)
    # We need a unique key. Since we don't pass index, we hash the answer or use random
    # unique_key = hash(res_data['answer']) # Simple hash
    # render_copy_button(res_data['answer'], unique_key) 
    # ^ Wait, if I call it here it breaks the HTML structure of the card (the card div is still open).
    # Solution: We can't easily embed the iframe INSIDE the div across st calls.
    # We have to close the card content div, render copy button, then continue?
    # Or just don't put copy button inside the big HTML string?
    
    # Let's adjust the strategy: 
    # Render the copy button is handled by main.py OUTSIDE this function to be safe? 
    # OR, we close the <div> in the first markdown block, but that splits the card style.
    
    # Best Approach for Visuals: 
    # Keep the card purely for visual container. The copy button can sit slightly outside or we accept it's an iframe block.
    # Let's NOT call render_copy_button inside here to avoid layout break.
    # I will just close the <div> normally.
    pass 
    
    # Reverting to original logic for the function body, but just inserting the new function above it.
    
    # 2. Citations Section (Inside the card but distinct)
    if res_data.get("citations"):
        st.markdown(f"<div style='padding: 0 25px 20px 25px;'>", unsafe_allow_html=True)
        st.markdown(f"<div style='margin-bottom: 8px; font-size: 0.85rem; opacity: 0.8; font-weight: 600;'>📚 เอกสารอ้างอิง ({len(res_data['citations'])}):</div>", unsafe_allow_html=True)

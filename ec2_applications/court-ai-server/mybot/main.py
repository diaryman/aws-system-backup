...
def render_combined_feedback_form(key_prefix, username, prompt, res_l, res_r, is_latest=False):
    ...
    with st.expander("✨ ให้คะแนนและข้อเสนอแนะ (Feedback)", expanded=st.session_state[expander_key]):
        ...
        with sc1:
            suggestion_l = st.text_area(
                f"ความคิดเห็นต่อฝั่งซ้าย ({res_l['model']})",
                placeholder="ระบุจุดที่ผิดหรือคำตอบที่ควรเป็น...",
                height=120, key=f"{key_prefix}_text_l"
            )
        with sc2:
            suggestion_r = st.text_area(
                f"ความคิดเห็นต่อฝั่งขวา ({res_r['model']})",
                placeholder="ระบุจุดที่ผิดหรือคำตอบที่ควรเป็น...",
                height=120, key=f"{key_prefix}_text_r"
            )
        ...
        with sub_col:
            if st.button("💾 บันทึกการประเมิน (Submit)", key=f"btn_sub_{key_prefix}", use_container_width=True):
                ...
                fb_l = {"accuracy": get_score(st.session_state.get(f"{key_prefix}_l_acc")), "completeness": get_score(st.session_state.get(f"{key_prefix}_l_comp")), "details": get_score(st.session_state.get(f"{key_prefix}_l_det")), "usefulness": get_score(st.session_state.get(f"{key_prefix}_l_use")), "satisfaction": get_score(st.session_state.get(f"{key_prefix}_l_sat")), "suggestion": suggestion_l}
                fb_r = {"accuracy": get_score(st.session_state.get(f"{key_prefix}_r_acc")), "completeness": get_score(st.session_state.get(f"{key_prefix}_r_comp")), "details": get_score(st.session_state.get(f"{key_prefix}_r_det")), "usefulness": get_score(st.session_state.get(f"{key_prefix}_r_use")), "satisfaction": get_score(st.session_state.get(f"{key_prefix}_r_sat")), "suggestion": suggestion_r}
...
def generate_admin_word_report(df, df_metrics):
    ...
    stats_data = [
        ...
        ("จำนวนผู้ใช้งานรวมตามกลุ่ม", f"{len(df):,} คน"),
...
def parse_feedback_scores(fb_str):
    ...
    suggestion = ""
    if "ข้อเสนอแนะเพิ่มเติม:" in fb_str:
        parts = fb_str.split("ข้อเสนอแนะเพิ่มเติม:", 1)
        fb_str_scores = parts[0]
        suggestion = parts[1].strip()
    elif " Suggestion: " in fb_str:
    ...
    scores = {}
    pattern1 = r"([A-Za-z]+):(\d+)"
    matches1 = re.findall(pattern1, fb_str_scores)
    ...
    pattern2 = r"([1-5]\.\s*[^:]+):\s*([0-5])\s*⭐"
    matches2 = re.findall(pattern2, fb_str_scores)
    for k, v in matches2:
...
def render_dashboard(current_username):
    ...
    # This deployment is currently an evaluation environment. Synthetic persona
    # runs are included in all dashboards, while their flags and run IDs remain
    # available for audit, filtering and future separation before production.

    # Normalize both current and historical backend names into the two
    # executive-facing model groups used by this application.
    def normalize_model_group(value):
        name = str(value or "").strip()
        lowered = name.lower()
        if any(token in lowered for token in ("gemini", "nova")):
            return "Gemini"
        if "claude" in lowered:
            return "Claude"
        return name

    all_df['Model_Left'] = all_df['Model_Left'].apply(normalize_model_group)
    all_df['Model_Right'] = all_df['Model_Right'].apply(normalize_model_group)
    allowed_models = {"Gemini", "Claude"}
    all_df = all_df[
        all_df['Model_Left'].isin(allowed_models) &
        all_df['Model_Right'].isin(allowed_models)
    ].copy()
    if all_df.empty:
        st.info("ยังไม่มีข้อมูลของกลุ่ม Gemini และ Claude สำหรับวิเคราะห์")
        return
    ...
    for side in ('Left', 'Right'):
        feedback_col = f'Feedback_{side}'
        for short_code, full_name in metric_codes.items():
            all_df[f'{short_code}_{side}'] = all_df[feedback_col].apply(
                lambda value, key=full_name: (parse_feedback_scores(value)[0] or {}).get(key, 0)
            )
        all_df[f'Suggestion_{side}'] = all_df[feedback_col].apply(
            lambda value: parse_feedback_scores(value)[1]
        )
    ...
    def extract_info(u):
        ...
        if " | " in u:
            parts = u.split(" | ")
            pos = parts[1].strip() if len(parts) > 1 else "ไม่ระบุตำแหน่ง"
            dept = parts[2].strip() if len(parts) > 2 else "ไม่ระบุหน่วยงาน"
            return pos, dept
        pos = u.split('(')[0].strip() if '(' in u else (u.split('-')[0].strip() if '-' in u else u)
        dept = u.split('-')[-1].strip() if '-' in u else "ไม่ระบุหน่วยงาน"
        return pos, dept
    ...
    # Evaluation environment: every submitted user record is counted, including
    # repeated position/level/agency combinations.
    unique_users = len(all_df)
    
    kpi1.metric("จำนวนคำถามทั้งหมด", f"{total_q:,}")
    kpi2.metric("จำนวนผู้ใช้งาน (รวมซ้ำ)", f"{unique_users:,}")
    ...
    tab_executive, tab_visuals, tab_battle, tab_users, tab_ai_insight, tab_raw = st.tabs([
        "🧭 สรุปผู้บริหาร (Executive)",
    ...
    with tab_executive:
        ...
        evaluated_sides = int(all_df['Feedback_Left'].fillna('').ne('').sum() + all_df['Feedback_Right'].fillna('').ne('').sum())
        total_sides = max(len(all_df) * 2, 1)
        feedback_rate = evaluated_sides / total_sides * 100

        score_values = pd.concat([
            all_df[['Acc_Left', 'Comp_Left', 'Det_Left', 'Use_Left', 'Sat_Left']].stack(),
            all_df[['Acc_Right', 'Comp_Right', 'Det_Right', 'Use_Right', 'Sat_Right']].stack()
        ])
        score_values = pd.to_numeric(score_values, errors='coerce')
        overall_quality = score_values[score_values > 0].mean() if (score_values > 0).any() else 0

        risk_left = ((all_df['Acc_Left'].between(1, 2)) | (all_df['Sat_Left'].between(1, 2))).sum()
        risk_right = ((all_df['Acc_Right'].between(1, 2)) | (all_df['Sat_Right'].between(1, 2))).sum()
        high_risk_count = int(risk_left + risk_right)
        risk_rate = high_risk_count / max(evaluated_sides, 1) * 100
        avg_time = pd.concat([all_df['Time_Left'], all_df['Time_Right']]).mean()
        avg_cost = total_cost / total_sides

        ek1, ek2, ek3, ek4, ek5, ek6 = st.columns(6)
        ek1.metric("คำถาม", f"{total_q:,}")
        ek2.metric("ผู้ใช้งาน (รวมซ้ำ)", f"{unique_users:,}")
        ek3.metric("คุณภาพเฉลี่ย", f"{overall_quality:.2f}/5")
        ek4.metric("Feedback Rate", f"{feedback_rate:.1f}%")
        ek5.metric("ความเสี่ยงสูง", f"{risk_rate:.1f}%", help="Accuracy หรือ Satisfaction 1–2 ดาว")
        ek6.metric("เวลาเฉลี่ย", f"{avg_time:.1f} วินาที")
        st.caption(f"ต้นทุนเฉลี่ยต่อคำตอบ {avg_cost:.4f} บาท | เคสความเสี่ยงสูง {high_risk_count:,} คำตอบ")

        ex1, ex2 = st.columns(2)
        with ex1:
            st.markdown("#### แนวโน้มการใช้งานและคุณภาพ")
            trend = all_df.groupby(all_df['dt_timestamp'].dt.date).agg(
                Questions=('Question', 'count'),
                Satisfaction_Left=('Sat_Left', lambda x: x[x > 0].mean()),
                Satisfaction_Right=('Sat_Right', lambda x: x[x > 0].mean())
            ).reset_index().rename(columns={'dt_timestamp': 'Date'})
            fig_exec_trend = px.line(
                trend, x='Date',
                y=['Questions', 'Satisfaction_Left', 'Satisfaction_Right'],
                markers=True
            )
            st.plotly_chart(fig_exec_trend, use_container_width=True)
        with ex2:
            st.markdown("#### คุณภาพและความคุ้มค่ารายโมเดล")
            model_rows = []
            for side in ('Left', 'Right'):
                for _, row in all_df.iterrows():
                    sat = row[f'Sat_{side}']
                    model_rows.append({
                        ...
                        'Satisfaction': sat if sat > 0 else None,
                        'Time': row[f'Time_{side}'],
                        'Cost': row[f'Cost_Model_{side}']
                    })
            model_exec = pd.DataFrame(model_rows).groupby('Model').agg(
                Satisfaction=('Satisfaction', 'mean'),
                Avg_Time=('Time', 'mean'),
                Avg_Cost=('Cost', 'mean'),
                Responses=('Model', 'count')
            ).reset_index()
            st.dataframe(
                model_exec.sort_values('Satisfaction', ascending=False),
                use_container_width=True,
                hide_index=True
            )

        st.markdown("#### ประเด็นเร่งด่วนสำหรับผู้บริหาร")
        executive_risks = []
        for _, row in all_df.iterrows():
            for side in ('Left', 'Right'):
                if row[f'Acc_{side}'] in (1, 2) or row[f'Sat_{side}'] in (1, 2):
                    executive_risks.append({
                        'เวลา': row['Timestamp'],
                        'โมเดล': row[f'Model_{side}'],
                        'คำถาม': row['Question'],
                        'Accuracy': row[f'Acc_{side}'],
                        'Satisfaction': row[f'Sat_{side}'],
                        'ความคิดเห็น': row[f'Suggestion_{side}']
                    })
        if executive_risks:
            st.dataframe(pd.DataFrame(executive_risks), use_container_width=True, height=360, hide_index=True)
        else:
    ...
    with tab_users:
        ...
        group_total = int(all_df['Position'].value_counts().sum())
        st.metric("จำนวนผู้ใช้งานรวมจากทุกกลุ่ม", f"{group_total:,}")
        st.caption("ผลรวมสมาชิกทุกกลุ่มใช้ฐานข้อมูลเดียวกับ KPI จำนวนผู้ใช้งาน จึงต้องมีค่าเท่ากัน")
        u1, u2 = st.columns(2)
        ...
        for fb in pd.concat([user_data['Feedback_Left'], user_data['Feedback_Right']]):
            ...
            sat_key = "5. Overall Satisfaction (ความพึงพอใจและประโยชน์ใช้งาน)"
            if scs and sat_key in scs:
                sat_scores.append(scs[sat_key])
    ...
    with tab_battle:
        ...
        if not scored_df.empty:
            ...
            battle_history = scored_df[['Timestamp', 'Question', 'Model_Left', 'Sat_Left', 'Model_Right', 'Sat_Right', 'Winner']].iloc[::-1]
            st.dataframe(battle_history, use_container_width=True, height=420)
        else:
    ...
    with tab_raw:
        ...
        st.markdown("### 📥 ดาวน์โหลดผลการประเมินจากฐานข้อมูล")
        st.caption("ส่งออกข้อมูลจากฐานของระบบโดยตรง ไฟล์ UTF-8 รองรับภาษาไทย และสามารถนำไปคำนวณต่อใน Excel, SPSS, R หรือ Python ได้")
        try:
            import sqlite3
            with sqlite3.connect("data/logs.db") as eval_conn:
                evaluation_df = pd.read_sql_query("""
                    SELECT
                        ...
                        f.rubric_version,
                        f.created_at,
                        CASE WHEN f.response_side = left THEN l.cost_left ELSE l.cost_right END AS cost,
                        CASE WHEN f.response_side = left THEN l.time_left ELSE l.time_right END AS response_time
                    FROM feedback_evaluations f
                    INNER JOIN logs l ON l.id = f.log_id
                    ORDER BY l.id, f.id
                """, eval_conn)

            ev1, ev2 = st.columns(2)
            ev1.download_button(
                label=f"📊 ดาวน์โหลดคะแนนประเมิน CSV ({len(evaluation_df):,} ระเบียน)",
                data=evaluation_df.to_csv(index=False).encode("utf-8-sig"),
                file_name="smart_court_evaluation_scores.csv",
                mime="text/csv",
                use_container_width=True,
                key="download_evaluation_scores_csv"
            )

            answers_df = all_df.copy()
            if dt_timestamp in answers_df.columns:
                answers_df = answers_df.drop(columns=[dt_timestamp])
            ev2.download_button(
                label=f"🗂️ ดาวน์โหลดข้อมูลพร้อมคำตอบ CSV ({len(answers_df):,} รายการ)",
                data=answers_df.to_csv(index=False).encode("utf-8-sig"),
                file_name="smart_court_evaluations_with_answers.csv",
                mime="text/csv",
                use_container_width=True,
                key="download_evaluations_with_answers_csv"
            )
        except Exception as export_error:
            st.error(f"ไม่สามารถส่งออกผลการประเมินได้: {export_error}")
...
if not st.session_state.username_confirmed:
    ...
    with c2:
        ...
        with st.container(border=True):
            ...
            if selected_pos == "อื่นๆ":
                dept_options.append("อื่นๆ")

            selected_dept = st.selectbox("หน่วยงาน", dept_options, key="login_dept")

            final_dept = selected_dept
            if selected_dept == "อื่นๆ":
                final_dept = st.text_input(
                    "ระบุชื่อหน่วยงานอื่นๆ",
                    placeholder="โปรดระบุ...",
                    key="login_dept_other"
                )
            ...
            if st.button("🚀 เข้าสู่ระบบ (Start)", type="primary", use_container_width=True):
                ...
                elif selected_dept == "อื่นๆ" and not final_dept.strip():
                    st.warning("⚠️ กรุณาระบุชื่อหน่วยงานของคุณ")
                else:
                    # Construct Username: Position (Level) - Department
                    user_parts = [final_pos]
                    if selected_level:
                        user_parts.append(f"({selected_level})")
                    user_parts.append(f"- {final_dept.strip()}")
...
with st.sidebar:
    ...
    if page_mode == "💬 แชท (Chat)":
        st.markdown("---")
        st.markdown("##### ⚔️ กลุ่มเปรียบเทียบ Gemini + Claude")
        st.caption("ระบบเปรียบเทียบคำตอบจาก Gemini และ Claude โดยใช้คลังข้อมูลเดียวกัน")
        with st.container(border=True):
            kb_left_name = st.selectbox("คลังข้อมูลสำหรับทั้งสองโมเดล", list(KNOWLEDGE_BASES.keys()), index=0, key="kb_group")
            kb_right_name = kb_left_name
            mc1, mc2 = st.columns(2)
            mc1.info("👈 Gemini")
            mc2.success("👉 Claude 4.6 Sonnet")
            m_left = "Gemini"
            m_right = "Claude 4.6 Sonnet"
            ...
            with st.chat_message("assistant", avatar="⚖️"):
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    ...
                    while not (done_l and done_r):
                        ...
                        if ph_l and updated_l:
                            ph_l.markdown(ans_l + "▌")
                        if ph_r and updated_r:
                            ph_r.markdown(ans_r + "▌")
